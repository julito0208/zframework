...
class HTTPFiles {
	...
	public static function has_file($name, $index=null)
	{
		return $_FILES[$name] && $_FILES[$name];
	}

	public static function control_file($name, $index=null)
	{
		if(!is_null($index))
		{
			if(!$_FILES[$name]['name'][$index] || $_FILES[$name]['error'][$index] || $_FILES[$name]['size'][$index] == 0)
			{
				return false;
			}
		...
		if(!$_FILES[$name] || $_FILES[$name]['error'] || $_FILES[$name]['size'] == 0)
		{
			return false;
		}
	...
	public static function get_dict() {
		if(is_null(self::$_VARS)) {
			self::$_VARS = new DictRead($_FILES);
		}
