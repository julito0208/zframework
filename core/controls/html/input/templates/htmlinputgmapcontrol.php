<div class="gmap-control-block" style="margin: 0 auto; <? if($width) echo 'width: '.(is_numeric($width) ? ($width+2).'px' : "{$width}").'; '; ?>">
	...
	<? if($enable_change_type) { ?>
		...
		<div class="gmap-type-controls">
			<a href="javascript: void(0)" title="<?=HTMLHelper::escape(String::get('gmap_view_type_roadmap'))?>" onclick="<?=$varname?>.setMapType('<?=HTMLInputGmapControl::GMAP_TYPE_ROADMAP?>'); $(this).parent().children().removeClass('selected'); $(this).addClass('selected');"<? if($map_type == HTMLInputGmapControl::GMAP_TYPE_ROADMAP) echo " class='selected'";?>><?=HTMLHelper::escape(String::get('gmap_view_type_roadmap'))?></a>
			<a href="javascript: void(0)" title="<?=HTMLHelper::escape(String::get('gmap_view_type_satellite'))?>" onclick="<?=$varname?>.setMapType('<?=HTMLInputGmapControl::GMAP_TYPE_SATELLITE?>'); $(this).parent().children().removeClass('selected'); $(this).addClass('selected');"<? if($map_type == HTMLInputGmapControl::GMAP_TYPE_SATELLITE) echo " class='selected'";?>><?=HTMLHelper::escape(String::get('gmap_view_type_satellite'))?></a>
			<a href="javascript: void(0)" title="<?=HTMLHelper::escape(String::get('gmap_view_type_hybrid'))?>" onclick="<?=$varname?>.setMapType('<?=HTMLInputGmapControl::GMAP_TYPE_HYBRID?>'); $(this).parent().children().removeClass('selected'); $(this).addClass('selected');"<? if($map_type == HTMLInputGmapControl::GMAP_TYPE_HYBRID) echo " class='selected'";?>><?=HTMLHelper::escape(String::get('gmap_view_type_hybrid'))?></a>
			<a href="javascript: void(0)" title="<?=HTMLHelper::escape(String::get('gmap_view_type_terrain'))?>" onclick="<?=$varname?>.setMapType('<?=HTMLInputGmapControl::GMAP_TYPE_TERRAIN?>'); $(this).parent().children().removeClass('selected'); $(this).addClass('selected');"<? if($map_type == HTMLInputGmapControl::GMAP_TYPE_TERRAIN) echo " class='selected'";?>><?=HTMLHelper::escape(String::get('gmap_view_type_terrain'))?></a>
		</div>
