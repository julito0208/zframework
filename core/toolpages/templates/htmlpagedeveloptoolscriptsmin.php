...
<script type="text/javascript">
	...
	function submitMinForm(action, confirmDialog) {
		...
		if(minFiles.length == 0) {
			...
			$.modalDialog.alert('No seleccion� ning�n archivo', 'error');
			
		} else {
		
			if(confirmDialog) {
				
				$.modalDialog.confirm(action == 'delete' ? '�Est� seguro que desea eliminar los mimificados de estos archivos?' : '�Est� seguro que desea mimificar estos archivos?', function() {
