...
$.fn.htmlbox=function(options){
	...
	var toolbar=function(){
		...
		for(var k=0;k<d.toolbars.length;k++){
			...
			for(var i=0;i<(toolbar.length);i++){
				...
				if(in_array(toolbar[i],cmds)){h += "<td class='"+d.id+"_html_button  html_button' valign='middle' align='center' onclick='global_hb[\""+d.id+"\"].cmd(\""+toolbar[i]+"\")' title='"+cmds[toolbar[i]]+"'><image src='"+img+"'></td>";}
		    }
			h += "</table></td></tr>";
		}
		return h;
	...
	this._init = function(is_init){
		...
		var w=$(this).css("width");var h=$(this).css("height");$(this).wrap("<table class='htmlbox-table' id='"+d.id+"_wrap' width='"+w+"' style='height:"+h+";border:2px solid #E9EAEF;' cellspacing='0' cellpadding='0'><tr><td id='"+d.id+"_container'></td></tr></table>");
