...
$.fn.htmlbox=function(options){
	...
	var d={
		...
		idir: _zphp['zframework_dir'] + '/thirdparty/htmlbox/images',
		icons:"default",  // Icon set
		about:true,
		//skin:"default",  // Skin, silver
		skin: 'blue',
		output:"xhtml",  // Output
		toolbar_height:28,// Toolbar height
		tool_height:15,   // Tools height
		tool_width:16,    // Tools width
		tool_image_height:15,  // Tools image height
		tool_image_width:16,  // Tools image width
		//css:"body{margin:3px;font-family:verdana;font-size:11px;}p{margin:0px;}",
		css:"body{margin:3px;font-family:verdana;font-size:11px;}p{margin:0px;} body{font-size: 9pt; font-family: serif; margin: 0 auto }",		
...
//		    d.css=d.css+"body{background-image:url("+d.idir+"logo.gif);background-position:top right;background-repeat:no-repeat;}";
		    d.css=d.css+"body{background:transparent}";
		}
	...
	var toolbar=function(){
		...
		for(var k=0;k<d.toolbars.length;k++){
			...
			for(var i=0;i<(toolbar.length);i++){
	            ...
	            // END: Custom button
				else if(toolbar[i]==="separator"){h += "<td valign='middle' align='center' class='htmlbox-separator'><image src='"+d.idir+"separator.gif' style='margin-right:3px;margin-left:3px;height:13px;'></td>";}
				else if(toolbar[i]==="fontsize"){
				    h += "<td valign='middle' align='center'><select id='"+d.id+"_fontsize' onchange='global_hb[\""+d.id+"\"].cmd(\"fontsize\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px;'><option value='' style='font-weight: bold' selected>Tamaño&nbsp;&nbsp;</option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option><option value='6'>6</option><option value='7'>7</option></select></td>";
			    }else if(toolbar[i]==="fontfamily"){
				    h += "<td valign='middle' align='center'><select id='"+d.id+"_fontfamily' onchange='global_hb[\""+d.id+"\"].cmd(\"fontname\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px;'><option value='' style='font-weight: bold' selected>Fuente</option><option value='arial' style='font-family:arial;'>Arial</option><option value='courier' style='font-family:courier;'>Courier</option><option value='cursive' style='font-family:cursive;'>Cursive</option><option value='georgia' style='font-family:georgia;'>Georgia</option><option value='monospace' style='font-family:monospace;'>Monospace</option><option value='tahoma' style='font-family:tahoma;'>Tahoma</option><option value='verdana' style='font-family:verdana;'>Verdana</option></select></td>";
				}else if(toolbar[i]==="formats"){
				 	//h += "<td valign='middle' align='center' style='padding:0'><select id='"+d.id+"_formats' onchange='global_hb[\""+d.id+"\"].cmd(\"format\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px;'><option value='' selected>- FORMATS -</option><option value='h1'>Heading 1</option><option value='h2'>Heading 2</option><option value='h3'>Heading 3</option><option value='h4'>Heading 4</option><option value='h5'>Heading 5</option><option value='h6'>Heading 6</option><option value='p'>Paragraph</option><option value='pindent'>First Indent</option><option value='pre'>Preformatted</option></select></td>";
				 	h += "<td valign='middle' align='center' style='padding:0;'><select id='"+d.id+"_formats' onchange='global_hb[\""+d.id+"\"].cmd(\"format\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px; vertical-align:middle; background: #FFF'>";
				 	h += "<option value='' style='font-weight: bold' selected>Formatos</option>";
				 	h += "<option value='h1'>Titulo 1</option>";
				 	h += "<option value='h2'>Titulo 2</option>";
				 	h += "<option value='h3'>Titulo 3</option>";
				 	h += "<option value='h4'>Titulo 4</option>";
				 	h += "<option value='h5'>Titulo 5</option>";
				 	h += "<option value='h6'>Titulo 6</option>";
				 	h += "<option value='p'>Parrafo</option>";
				 	h += "</select></td>";
				 	
				}else if(toolbar[i]==="fontcolor"){
				    h += "<td valign='middle' align='center'><select id='"+d.id+"_fontcolor' onchange='global_hb[\""+d.id+"\"].cmd(\"fontcolor\",this.options[this.selectedIndex].value);this.options[0].selected=\"true\";' style='font-size:12px;'><option value='' style='font-weight: bold' selected>Color &nbsp; &nbsp; </option>";
					for(var m=0;m<colors.length;m++){ if(m%2){continue;}
					   h+="<option value='"+colors[m]+"' style='background:"+colors[m]+";color:"+colors[m]+";'> &nbsp; &nbsp; </option>";
					}
					h += "</select></td>";
				...
				if(in_array(toolbar[i],cmds)){h += "<td class='"+d.id+"_html_button html_button' valign='middle' align='center'><a href='javascript:void(0)' onclick='global_hb[\""+d.id+"\"].cmd(\""+toolbar[i]+"\")' title='"+cmds[toolbar[i]]+"' style='display: block'><image style='border:0; vertical-align:middle' src='"+img+"'></a></td>";}
		    }
			h += "</table></td></tr>";
		}
		return h;
	...
	this._init = function(is_init){
		...
		var w=$(this).css("width");var h=$(this).css("height");$(this).wrap("<table class='htmlbox-table' id='"+d.id+"_wrap' width='"+w+"' style='height:"+h+";border:2px solid #E9EAEF;' cellspacing='0' cellpadding='0'><tr><td id='"+d.id+"_container' class='htmlbox-text-container' style='padding: 5px; cursor: text'></td></tr></table>");
		// START: Appending toolbar
		
		var toolbart = $(toolbar());
		toolbart.find('td,th').css('padding','0');
		$(this).parent().parent().parent().parent().prepend(toolbart);
		...
		*/
		...
		var button_over_style = {
			border: 'solid 1px #AAA',
			background: '#EFF2FF'};
		
		$("."+d.id+"_html_button").find('a').each(function(){
			// Set tool dimension
		    ...
		    // Set borders
		    $(this).css(button_style).css({
		    	margin: '1px 4px',
		    	padding: '1px'});

		    $(this).mouseover(function(){$(this).css(button_over_style);});
			$(this).mouseout(function(){$(this).css(button_style);});
			}
		);

		// FIN EDITADO ----------------------------------------------------------------------------------------------		
		
		$("#"+d.id+"_wrap").css("border",hb_border).css('cursor', 'text');
		$("#"+d.id+"_wrap").css("background",hb_background).css('cursor', 'text');
		$("#"+d.id+"_container").css("background","white").css('cursor', 'text');
		$("."+d.id+"_tb").css("border-bottom",tb_border).css('cursor', 'text').addClass('htmlbox-header');
		...
		try {
		   ...
		   $(iframe).css("width",w).css("height",h).attr("id",d.id+"_html").css("border","0").css("width", "100%").css("height", "100%");
		   $(this).parent().prepend(iframe);
		   // START: Shortcuts for less code
		   //d.css+='body { cursor: text } ';
		   d.iframe = iframe;
...
//		       d.css=d.css+"body{background-image:url("+d.idir+"logo.gif);background-position:top right;background-repeat:no-repeat;}";
	...
	this.cmd = function(cmd,arg1){
	   ...
	   if(cmd==="code"){
		   ...
		   var urlLink = prompt('Ingrese la dirección:');
		   if(urlLink) d.idoc.execCommand("createlink", false, urlLink);
	   }else if(cmd==="image"){
	...
	*/
	 ...
	 this.get_text = function() {
	 	...
	 	var wrapTag = $('<div />').css({display: 'none', position: 'absolute'}).appendTo(document.body);
	 	var textParts = [];
	 		 	
	 	$.each(this.get_html().split(typeof getXHTML === 'function' ? '<br />' : '<br>'), function(index, html){
	 		textParts.push(wrapTag.html(html).text());
