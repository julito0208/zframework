...
tinymce.PluginManager.add('autolink',function(editor){var AutoUrlDetectState;editor.on("keydown",function(e){if(e.keyCode==13){return handleEnter(editor);}});if(tinymce.Env.ie){editor.on("focus",function(){if(!AutoUrlDetectState){AutoUrlDetectState=true;try{editor.execCommand('AutoUrlDetect',false,true);}catch(ex){}}});return;}
...
return index;}
function setStart(container,offset){rng.setStart(container,scopeIndex(container,offset));}
function setEnd(container,offset){rng.setEnd(container,scopeIndex(container,offset));}
...
text=rng.toString();matches=text.match(/^(https?:\/\/|ssh:\/\/|ftp:\/\/|file:\/|www\.|(?:mailto:)?[A-Z0-9._%+\-]+@)(.+)$/i);if(matches){if(matches[1]=='www.'){matches[1]='http://www.';}else if(/@$/.test(matches[1])&&!/^mailto:/.test(matches[1])){matches[1]='mailto:'+matches[1];}
